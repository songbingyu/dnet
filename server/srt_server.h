...
typedef struct svr_tcb {
	...
	/* Variables to keep track of server-client connection */
...
/* Global Server TCB table array that holds a TCB table for each client-server connection */

svr_tcb_t * svrConnList [MAX_TRANSPORT_CONNECTIONS];

/* Actual socket file descriptor created in overlay_start that contains the tcp connection with the app_client */
